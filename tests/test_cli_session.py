...
class CliSessionTests(unittest.TestCase):
    ...
    def test_node_list_command_uses_saved_session_and_filters(self):
        import ftp_project
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            with patch("ftp_project.SessionStore", return_value=store), patch(
                ...
                client_type.return_value.get.return_value = {
                    "items": [{"id": "node-1", "status": "ONLINE"}],
                }

                self.assertEqual(
                    ftp_project.main(
                        ["node", "list", "--status", "ONLINE", "--capability", "web"]
                    ),
                    0,
                )
                ...
                client_type.return_value.get.assert_called_once_with(
                    "/v1/nodes?status=ONLINE&capability=web"
                )

    def test_node_heartbeat_command_uses_saved_session(self):
        import ftp_project
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            with patch("ftp_project.SessionStore", return_value=store), patch(
                ...
                client_type.return_value.create.return_value = {
                    "id": "node-1",
                    "status": "DEGRADED",
                }
    ...
    def test_service_get_command_uses_saved_session(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            with patch("ftp_project.SessionStore", return_value=store), patch(
                ...
                client_type.return_value.get.return_value = {
                    "id": "service-1", "type": "WEB", "lifecycle_state": "ACTIVE"
                }

                self.assertEqual(ftp_project.main(["service", "get", "service-1"]), 0)

                client_type.assert_called_once_with(session, base_url="http://localhost:8000")
                client_type.return_value.get.assert_called_once_with("/v1/services/service-1")

    def test_service_state_get_command_uses_saved_session(self):
        import ftp_project
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            with patch("ftp_project.SessionStore", return_value=store), patch(
                ...
                client_type.return_value.get.return_value = {
                    "desired": {"version": 1}, "assignment": None, "actual": None
                }

                self.assertEqual(
                    ftp_project.main(["service", "state", "get", "service-1"]), 0
                )
